...
def reiniciar_juego(nivel_seleccionado):
    ...
    global flag_ganador, ganador_auto_principal, tiempo_inicio
    ...
    from constantes import flag_ganador, ganador_auto_principal  # Importar explícitamente
...
def jugar():
    ...
    while True:  # 🔄 Este bucle permite reiniciar el juego sin volver al menú
        ...
        while cerrar_ventana():
            ...
            if flag_ganador_ and tiempo_inicio is not None and pygame.time.get_ticks() - tiempo_inicio >= 3000:
                #print("⏳ Esperamos 3 segundos antes de mostrar la pantalla de resultados...")
                accion = mostrar_pantalla_resultado(ventana, ganador_auto_principal_, ranking_ejemplo)
                break
